...
- (void)loadView {
	...
	self.tabBar = [[[BCTabBar alloc] initWithFrame:CGRectMake(0, self.view.bounds.size.height - 57,
															  self.view.bounds.size.width, 57)]
				   autorelease];
...
- (void)tabBar:(BCTabBar *)aTabBar didSelectTabAtIndex:(NSInteger)index {
	...
	[bctabBarDelegate tabBar:aTabBar didSelectTabAtIndex:index];
}
...
- (void)setSelectedViewController:(UIViewController *)vc {
	...
	if (selectedViewController != vc) {
        ...
        //add
        [self.tabBarView removeLastView];
		self.tabBarView.contentView = vc.view;
        //add
        [self.tabBarView addCurrentView];
		if (visible) {
